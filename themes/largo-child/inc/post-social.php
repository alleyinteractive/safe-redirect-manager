...
// Let's remove the parent action
add_action( 'wp_head', 'remove_floating_social_action' );
function remove_floating_social_action(){
    remove_action( 'wp_footer', 'largo_floating_social_button_js' );
}
 ...
 * @global LARGO_DEBUG
 */
if ( ! function_exists('onsomething_floating_social_button_js') ) {
	function onsomething_floating_social_button_js() {

        // Let's load our child version
		if ( is_single() && of_get_option('single_floating_social_icons', '1') == '1' ) {
			?>
			<script type="text/javascript" src="<?php
				echo get_stylesheet_directory_uri() . '/js/floating-social-buttons.js';
			?>" async></script>
			<?php
		}
	}
}
add_action('wp_footer', 'onsomething_floating_social_button_js');
